...
export class QualityAnalyzer {
  ...
  async startSalesAnalysis(artistInfo, data, currentWarnings, currentScore) {
    ...
    const candidateTerms = this.searchFilterManager.extractCandidateSearchTerms(data.title, data.description, artistInfo);
    if (candidateTerms) {
  ...
  generateFreetextSearch(title, description, artistInfo = null) {
    ...
    const isJewelry = this.itemTypeHandlers.isJewelryItem(objectType, title, description);
    
    // NEW: Special handling for coins/numismatics
    const isCoin = this.itemTypeHandlers.isCoinItem(objectType, title, description);
    
    // NEW: Special handling for stamps/philatelic items
    const isStamp = this.itemTypeHandlers.isStampItem(objectType, title, description);
    
    // NEW: Special handling for audio/electronics equipment
    const isAudio = this.itemTypeHandlers.isAudioEquipment(objectType, title, description);
    
    // NEW: Special handling for watches/timepieces  
    const isWatch = this.itemTypeHandlers.isWatchItem(objectType, title, description);
    
    // NEW: Special handling for musical instruments
    const isInstrument = this.itemTypeHandlers.isMusicalInstrument(objectType, title, description);
    ...
    if (isJewelry) {
      ...
      return this.itemTypeHandlers.generateJewelrySearch(objectType, title, description, artistInfo, searchTerms, confidence);
    }
    
    if (isCoin) {
      console.log('🪙 Detected coin/numismatic item - using coin-specific search strategy');
      return this.itemTypeHandlers.generateCoinSearch(objectType, title, description, artistInfo, searchTerms, confidence);
    }
    
    if (isStamp) {
      console.log('📜 Detected stamp/philatelic item - using stamp-specific search strategy');
      return this.itemTypeHandlers.generateStampSearch(objectType, title, description, artistInfo, searchTerms, confidence);
    }
    
    if (isAudio) {
      console.log('🎵 Detected audio/electronics item - using audio-specific search strategy');
      return this.itemTypeHandlers.generateAudioSearch(objectType, title, description, artistInfo, searchTerms, confidence);
    }
    
    if (isWatch) {
      console.log('⌚ Detected watch/timepiece item - using watch-specific search strategy');
      return this.itemTypeHandlers.generateWatchSearch(objectType, title, description, artistInfo, searchTerms, confidence);
    }
    
    if (isInstrument) {
      console.log('🎼 Detected musical instrument - using instrument-specific search strategy');
      return this.itemTypeHandlers.generateMusicalInstrumentSearch(objectType, title, description, artistInfo, searchTerms, confidence);
    }
